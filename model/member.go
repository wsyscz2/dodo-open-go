...
type (
	GetMemberDodoIdMapListReq struct {
		DodoIdList []string `json:"dodoIdList" binding:"required"` //DoDo号列表，每次最多可查询1000个
	}

	MemberDodoIdMap struct {
		DodoId       string `json:"dodoId"`       // DoDo号
		DodoSourceId string `json:"dodoSourceId"` // DodoID
	}
)

func (p *GetMemberDodoIdMapListReq) ValidParams() error {
	if len(p.DodoIdList) <= 0 || len(p.DodoIdList) > 1000 {
		return errors.New("dodoIdList length out of range [0, 1000]")
	}
	return nil
}
